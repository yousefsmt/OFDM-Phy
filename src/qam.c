...
int map_bits_to_complex(const uint8_t* bits, complex_t* symbols)
{
    if (symbols && bits)
    {
        ...
        for (size_t i = 0; i < SYMBOL_RATE; i++)
        {
            for (size_t k = 0; k < BITS_PER_SYM / 2; k++)
            {
                switch (k)
                {
                    case 0:
                        ...
                        symbols[2 * i].i = (float) AMP_CALC((int) msb_four_bit, BITS_PER_SYM);
                        symbols[2 * i].q = (float) AMP_CALC((int) lsb_four_bit, BITS_PER_SYM);
                        break;
                    case 1:
                        ...
                        lsb_four_bit ^= lsb_four_bit >> 1;
                        symbols[2 * i + 1].i = (float) AMP_CALC((int) msb_four_bit, BITS_PER_SYM);
                        symbols[2 * i + 1].q = (float) AMP_CALC((int) lsb_four_bit, BITS_PER_SYM);
                        break;
                    default:
                        break;
                }
